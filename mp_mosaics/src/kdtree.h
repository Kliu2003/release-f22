...
class KDTree
{
  ...
  private:
     ...
     */
    void copy_(KDTreeNode*& thisNode, KDTreeNode*& otherNode);
    void delete_(KDTreeNode*& rootNode);

    int partition(vector<Point<Dim>>& newPoints, int low, int high, int pivotIndex, int currentDimension); //Partition of quickselect, returns storeIndex
    Point<Dim> select(vector<Point<Dim>>& newPoints, int low, int high, int k, int currentDimension); //Quickselect function, returns kth smallest point
    void generateKDTree(KDTreeNode*& root, vector<Point<Dim>>& newPoints, int low, int high, int currentDimension); //Helper function to generate KDTree

    Point<Dim> findNearestNeighborHelper(KDTreeNode* currNode, const Point<Dim> query, Point<Dim> currBest, int currDim) const; //Helper function to find nearest
